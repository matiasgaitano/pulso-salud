...
export function TriageChat({ consultationId, patientName }: Props) {
  ...
  const sendMessage = useCallback(
    ...
    [consultationId, isTyping, messages]
  );
    ...
    const greet = async () => {
      ...
      try {
        const res = await fetch("/api/triage", {
          ...
          body: JSON.stringify({
            consultationId,
            message: `Hola, soy ${patientName} y quiero una segunda opinión médica.`,
            history: [],
          }),
        });
        ...
        while (true) {
          ...
          for (const line of chunk.split("\n")) {
            ...
            try {
              ...
              if (parsed.type === "text") {
                fullText += parsed.text;
                setMessages([{ ...assistantMsg, content: fullText }]);
              }
            } catch { /* skip */ }
          }
        }
    ...
    return (
      <div className="flex flex-col items-center justify-center gap-6 py-12 text-center">
        ...
        <Button
          size="lg"
          onClick={() =>
            router.push(`/consultation/${consultationId}/specialist`)
          }
        >
